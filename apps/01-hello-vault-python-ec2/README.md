...
| **Vault Secret Engine** | KV v2 (`apps/hello-vault-python/config`) |
| **HCP Terraform Workspace** | `demo-app-01` |
...
│  (apps/hello-vault-python/config)                                                │
│    │                                                                             │
│    ▼                                                                             │
...
1. The EC2 instance uses its IAM instance profile to sign an `sts:GetCallerIdentity` request.
2. The Flask app sends the signed request headers to the Vault AWS Auth endpoint.
3. Vault validates the signature with AWS STS and checks the bound IAM role ARN.
4. Vault returns a short-lived token scoped to the `hello-vault-python-read` policy.
5. The app uses this token to retrieve `apps/hello-vault-python/config` from KV v2 and serves responses on port 8080.

## Quick Start

### 1. Configure Workspace Variables in HCP Terraform

Set the required variables in the `demo-app-01` workspace:
...
fqdn                   = "hello-python.christian-renaud.sbx.hashidemos.io"
```
...
curl http://hello-python.christian-renaud.sbx.hashidemos.io:8080/
curl http://hello-python.christian-renaud.sbx.hashidemos.io:8080/health
```
